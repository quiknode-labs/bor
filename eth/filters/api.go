...
func (api *PublicFilterAPI) GetLogs(ctx context.Context, crit FilterCriteria) ([]*types.Log, error) {
	...
	if crit.BlockHash != nil {
		...
		// Block bor filter
		if api.borLogs {
			borLogsFilter = NewBorBlockLogsFilter(api.backend, 64, *crit.BlockHash, crit.Addresses, crit.Topics)
		}
	} else {
		...
		// Block bor filter
		if api.borLogs {
			borLogsFilter = NewBorBlockLogsRangeFilter(api.backend, 64, begin, end, crit.Addresses, crit.Topics)
		}
	}
