...
func (api *PublicFilterAPI) GetBorBlockLogs(ctx context.Context, crit FilterCriteria) ([]*types.Log, error) {
	...
	if crit.BlockHash != nil {
		...
		filter = NewBorBlockLogsFilter(api.backend, 64, *crit.BlockHash, crit.Addresses, crit.Topics)
	} else {
		...
		filter = NewBorBlockLogsRangeFilter(api.backend, 64, begin, end, crit.Addresses, crit.Topics)
	}
